package com.example.cs.Gecko.rfq.utils;
...
public class ChatterboxServer {
    ...
    public static void runSender(String json) throws Exception {
...
//                    rfqSenderOutputThread = send(socket);
                    rfqSenderOutputThread = sendRFQ(socket);
    ...
    private static Thread sendRFQ(final Socket socket){
        return new Thread(() -> {
            try {
                do {
                    File file = new File("C:\\exercises\\Experimental\\cs-2021-eu-team-3\\src\\main\\resources\\rfqFile");
                    if(file.canRead()){
                        String dataRfq;
                        try(Stream<String> linesStream = Files.lines(file.toPath())){
                            dataRfq = linesStream.reduce("", (a,b) -> a + b);
                        }
                        PrintWriter out = new PrintWriter(new OutputStreamWriter(socket.getOutputStream()));
                        //naive polling of System.in to check for input and allow thread to be interrupted
                        out.println(dataRfq);
